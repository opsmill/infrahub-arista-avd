...
Add or keep one item under the existing `EVPN Services` group, alongside Tenants, SVIs, and L2 VLANs:
...
Do not add a direct `EvpnGatewayGroup` item under EVPN Services. Users should open an EVPN Domain and use its `local_gateway_groups` and `remote_gateway_groups` relationships to discover EVPN Gateway Groups.
...
- A user can open an EVPN Domain and discover local gateway group children plus remote gateway group references from the domain detail view.
