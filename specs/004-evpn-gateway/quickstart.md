...
- Validation commit for remote gates: `8808775ae4ba96fde1f2c6f08cd2f21800e13cae` on `feat/evpn-gateway`.
- Remote integration validation: `$infrahub-run-integration-tests` ran against `feat/evpn-gateway` at commit `8808775ae4ba96fde1f2c6f08cd2f21800e13cae` with `INFRAHUB_TESTING_DOCKER_IMAGE=opsmill/infrahub-solution-arista-avd`, `INFRAHUB_TESTING_IMAGE_VER=1.10.1`, and `INFRAHUB_TESTING_TASKMGR_BACKGROUND_SVC_REPLICAS=1`; `uv run pytest tests/integration` passed with 29 tests in 1014.75 seconds.
- Live generator idempotence validation: `$infrahub-test-generator-idempotence` rebuilt the shared live validation lab from a clean state, loaded Fabric-C seed data, confirmed `NetworkFabric Fabric-C` and `infrahub-dc1.parent -> Fabric-C`, synced `test-repository` to `feat/evpn-gateway` at commit `8808775ae4ba96fde1f2c6f08cd2f21800e13cae`, and ran on branch `idempotence-hostvar-20260723-1511`.
- Fabric-C idempotence scenario: `generate-fabric`, `generate-pod`, and `generate-rack` generated the Fabric-C topology; EVPN Domains DC1 (`65100:1`), DC2 (`65100:2`), and CORE (`65200:100`) were created under Fabric-C; `infrahub-dc1` and `infrahub-dc2` were assigned to their local domains; gateway groups `DC1-GW` and `DC2-GW` were created as local-domain-owned groups with CORE as `remote_domain`; four border leaves were assigned as members.
- Hostvar idempotence result: `generate-avd-device-hostvar` was run for `leaf-infrahub-dc1-1-1`, `leaf-infrahub-dc1-1-2`, `leaf-infrahub-dc2-1-1`, and `leaf-infrahub-dc2-1-2`, then run a second time for the same devices. The second run reported `Hostvars unchanged` for all four devices, storage IDs were unchanged, and normalized object-store hostvar JSON snapshots had `SNAPSHOT_DIFF=none`.
