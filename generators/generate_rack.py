...
class RackGenerator(InfrahubGenerator, GeneratorMixin):
    ...
    async def create_mlag_pairs(self) -> None:
        ...
        for pair_idx in range(0, len(self.leaf_switches) - 1, 2):
            ...
            desired_domain, legacy_domain = await self._get_existing_mlag_domains(domain_id)
            mlag_asn = await self._get_or_allocate_mlag_domain_asn(
                domain_id=domain_id,
                existing_domains=[domain for domain in (desired_domain, legacy_domain) if domain is not None],
                leaf_a=leaf_a,
                leaf_b=leaf_b,
            )

            # If this rack was previously generated by the old implementation,
            # migrate the legacy "mlag-<rack>" domain in place.  That avoids
            # leaving a duplicate stale MLAG domain around after the fixed run.
            domain_kwargs = {
                "domain_id": domain_id,
                "bgp_asn": mlag_asn,
            ...
            if desired_domain is None and legacy_domain is not None:
                domain_kwargs["id"] = legacy_domain.id
    ...
    async def _get_existing_mlag_domains(self, domain_id: str) -> tuple[object | None, object | None]:
        """Return current and legacy MLAG domains for a rack pair, if present."""
        desired_domains = await self.client.filters(kind="MlagDomain", domain_id__value=domain_id)
        legacy_domains = await self.client.filters(kind="MlagDomain", domain_id__value=f"mlag-{domain_id}")

        desired_domain = desired_domains[0] if desired_domains else None
        legacy_domain = legacy_domains[0] if legacy_domains else None
        return desired_domain, legacy_domain
        ...
        peer_asns = sorted(
            {
                asn
                for asn in (
                    self._number_attr_value(leaf_a, "bgp_asn"),
                    self._number_attr_value(leaf_b, "bgp_asn"),
                )
                if asn is not None and self._asn_is_in_pool(asn)
            }
        )
        for peer_asn in peer_asns:
            if peer_asn not in used_by_others:
                self.logger.info(
                    "MLAG domain %s: adopting existing peer ASN %s as the authoritative domain ASN",
                    domain_id,
                    peer_asn,
                )
                return peer_asn
