...
This guide is for **contributors** who want to extend, debug, or maintain the integration — adding device roles, transform outputs, schema fields, or fixing pipeline issues. It assumes familiarity with Python, GraphQL, and Infrahub generators/transforms.

## Start here

1. **[Architecture Overview](./architecture.md)** — system components, data model hierarchy, and the generator pipeline at a glance.
2. **[AVD Integration → Overview](./avd/overview.md)** — the two-phase pipeline (hostvars → structured config) and the pyAVD version target.
...
- **[Extending the Integration](./avd/extending.md)** — worked examples for new roles, new transform outputs, new hostvar fields.
...
If you want to *use* the system to provision fabrics and view configurations without modifying code, switch to the [user guide](/user-guide/).
